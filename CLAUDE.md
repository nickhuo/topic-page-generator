...
7. **render** — `render.build_page` assembles an `EventPage` (now carrying the full `need_plans`); `render.render_html` walks the plan in rank order, calls `blocks.module_to_block()` to adapt each assigned module into one of 7 `RenderBlock` shapes (`paragraph` / `timeline` / `chart` / `newsfeed` / `factsheet` / `map` / `reactions`), and renders a three-column layout (TOC nav + main content + reference rail) via Jinja2 templates under `templates/{chrome,needs,blocks}/`. Hero and countdown live in `templates/chrome/` as page chrome, not inside need sections; the `toc`, `reference`, `reference_timeline`, `reference_wikipedia`, and `sources_card` partials in `templates/chrome/` power the left TOC nav and the right reference rail.
