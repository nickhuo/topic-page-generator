...
7. **render** — `render.build_page` assembles an `EventPage` (now carrying the full `need_plans`); `render.render_html` walks the plan in rank order, calls `blocks.module_to_block()` to adapt each assigned module into one of 6 `RenderBlock` shapes (`paragraph` / `timeline` / `chart` / `newsfeed` / `factsheet` / `map`), and renders editorial single-column HTML via Jinja2 templates under `templates/{chrome,needs,blocks}/`. Hero and countdown live in `templates/chrome/` as page chrome, not inside need sections.
...
**Block layer** (`src/generator/blocks/`). `schema.py` defines the 6 `RenderBlock` shapes plus primitives (`NewsCard`, `TimelineEntry`, `Location`, `PullQuote`, etc.). `converter.py::module_to_block(module, sources, override)` adapts any `TypedModule` to a `RenderBlock`; templates only consume blocks, never raw module data. The default block kind per module is in `_DEFAULT_BLOCK_KIND`; a `need_plan.render_overrides[module_kind]` wins when set.
