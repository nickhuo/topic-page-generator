...
3. **research** — per-section research loop (`research.py`, `research_eval.py`, `prompts/research_query.py`, `prompts/research_eval.py`). Budgets: max 3 iterations per section, max 4 Tavily calls per section, MAX_TOTAL_TAVILY=50 globally. Wikidata + Wikipedia card fetched once and prepended to every section's pool.
